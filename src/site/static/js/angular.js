...
app.config(function($locationProvider, $mdIconProvider, $mdThemingProvider) {
  ...
  $mdThemingProvider.definePalette('arb-primary-theme',
  ...
  $mdThemingProvider.definePalette('arb-accent-theme',
  ...
  $mdThemingProvider.definePalette('arb-warn-theme',
...
app.controller('ArbitalCtrl', function($rootScope, $scope, $location, $timeout,
	$interval, $http, $compile, $anchorScroll, $mdDialog,
	userService, pageService, popoverService, urlService) {
    ...
    return function(data) {
      ...
      if (result.content) {
        ...
        var revealInterval = $interval(function() {
          ...
          var hiddenChildren = result.content.element
						.find('.reveal-after-render');
  ...
  $scope.getErrorFunc = function(urlPageType) {
    return function(data, status) {
      console.error('Error /json/' + urlPageType + '/:');
    ...
    for (var ruleIndex = 0; ruleIndex < urlRules.length; ruleIndex++) {
      ...
      if (matches) {
        ...
        for (var paramIndex = 0; paramIndex < parameters.length; paramIndex++) {
          ...
          args[parameter] = matches[paramIndex + 1];
        }
        ...
        if (rule == currentLocation.rule &&
						$scope.subdomain == currentLocation.subdomain) {
          var currentMatches = true;
          for (paramIndex = 0;
							paramIndex < parameters.length && currentMatches;
							paramIndex++) {
            var parameter = parameters[paramIndex];
            currentMatches =
							(args[parameter] == currentLocation.args[parameter]);
          }
          if (currentMatches) {
        ...
        $http({
          ...
          data: JSON.stringify({})
				...
				.success($scope.getSuccessFunc(function(data) {
  				...
  				return {
    				title: pageService.pageMap[$scope.subdomain].title +
							' - Private Domain',
    				content: $scope.newElement('<arb-group-index group-id=\'' +
							data.result.domainId +
				...
				.success($scope.getSuccessFunc(function(data) {
  				...
  				return {
    				...
    				content: $scope.newElement(
							'<arb-index featured-domains=\'::featuredDomains\'></arb-index>'),
  				};
				}))
        ...
        data: JSON.stringify(postData)
			...
			.success($scope.getSuccessFunc(function(data) {
  			...
  			return {
    			...
    			content: $scope.newElement(
						'<arb-admin-dashboard-page data=\'::adminDashboardData\'>' +
						'</arb-admin-dashboard-page>'),
  			};
			}))
        ...
        data: JSON.stringify(postData)
			...
			.success($scope.getSuccessFunc(function(data) {
  			...
  			return {
    			...
    			content: $scope.newElement(
						'<arb-dashboard-page ids-map=\'::dashboardPageIdsMap\'>' +
						'</arb-dashboard-page>'),
  			};
			}))
    ...
    pageService.loadEdit({
      ...
      success: $scope.getSuccessFunc(function() {
        ...
        return {
          ...
          content: $scope.newElement(
          '<arb-edit-page class=\'full-height\' page-id=\'' + pageId +
