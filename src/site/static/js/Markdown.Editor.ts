// jscs:disable
// needs Markdown.Converter.ts at the moment

import {HookCollection} from './Markdown.Converter.ts';
...
export function Editor(markdownConverter, idPostfix, options: any = {}) {
...
position.getPageSize = function() {
	...
	if (self.innerHeight && (self as any).scrollMaxY) {
		scrollWidth = doc.body.scrollWidth;
		scrollHeight = self.innerHeight + (self as any).scrollMaxY;
...
function UndoManager(callback, panels) {
	...
	var setMode = function(newMode, noSave = false) {
		if (mode != newMode) {
			mode = newMode;
			if (!noSave) {
				saveState();
	...
	var refreshState = function(isInitialState = false) {
...
function TextareaState(panels, isInitialState = false) {
	...
	this.setInputAreaSelectionStartEnd = function() {
		...
		if (!panels.ieCachedRange && (inputArea.selectionStart || inputArea.selectionStart === 0)) {
			...
			// we take it.
			var range = panels.ieCachedRange || doc.createRange();
...
function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions, getString) {
	...
	var inputBox = panels.input,
		buttons: any = {}; // buttons.undo, buttons.link, etc. The actual DOM elements.
	...
	function setupButton(button, isEnabled) {
		$(button).attr('disabled', isEnabled ? null : 'true');
		if (isEnabled) {
			if (!(button as any).isHelp) {
	...
	function makeSpritedButtonRow() {
		...
		var makeButton = function(id, title, xShift, textOp) {
			...
			if (textOp) (button as any).textOp = textOp;
	...
	if (chunk.endTag.length > 1 && chunk.startTag.length > 0) {
		...
		if (wrapType == 'image') {
			...
			var suffix = [
				...
				//'-knows: [bayes_rule],[bayes_rule_odds]',
				//'-wants: [bayes_rule],[bayes_rule_odds]',
				'path: [bayes_rule],[bayes_rule_odds]',
