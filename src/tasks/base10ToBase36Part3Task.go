...
func (task *Base10ToBase36Part3Task) Execute(db *database.DB) (delay int, err error) {
	...
	errMessage, err := db.Transaction(func(tx *database.Tx) (string, error) {
		...
		doOneQuery(db, `UPDATE pages SET privacyKey = privacyKeyBase36 WHERE 1;`)
		...
		doOneQuery(db, `UPDATE fixedIds SET pageId = pageIdBase36 WHERE 1;`)
