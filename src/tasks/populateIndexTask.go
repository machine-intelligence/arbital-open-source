...
type PageIndexDoc struct {
	...
	GroupId search.Atom `json:"groupId"`
}
...
func (task *PopulateIndexTask) Execute(c sessions.Context) (delay int, err error) {
	...
	err = database.QuerySql(c, `
		SELECT pageId,type,title,text,alias,groupId
		FROM pages
...
func populateIndexProcessPage(c sessions.Context, rows *sql.Rows) error {
	var pageId, groupId int64
	var alias string
	p := &PageIndexDoc{}
	if err := rows.Scan(&pageId, &p.Type, &p.Title, &p.Text, &alias, &groupId); err != nil {
	...
	p.GroupId = search.Atom(fmt.Sprintf("%d", groupId))
