...
func (task PropagateDomainTask) Execute(db *database.DB) (delay int, err error) {
	...
	err = propagateDomainsToPageAndDescendants(db, task.PageId, pageMap)
	if err != nil {
...
func _propagateDomainsToSinglePage(db *database.DB, pageId string) ([]interface{}, []interface{}, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("Faled to load pageDomainPairs: %v", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("Faled to load pageDomainPairs2: %v", err)
	}
	...
	if len(addDomainArgs) > 0 {
		...
		if _, err = statement.Exec(addDomainArgs...); err != nil {
			return nil, nil, fmt.Errorf("Failed to add to pageDomainPairs: %v", err)
		}
	}
	...
	if len(removeDomainArgs) > 0 {
		...
		if _, err = statement.Exec(args...); err != nil {
			return nil, nil, fmt.Errorf("Failed to remove pageDomainPairs: %v", err)
		}
	}

	return addDomainArgs, removeDomainArgs, nil
}

// propagateDomainsToPageAndDescendants forces domain recalculation for the given page and its children.
// It uses a depth-first search to process the children of the page.
func propagateDomainsToPageAndDescendants(db *database.DB, pageId string, pageMap map[string]bool) error {
	db.C.Infof("Processing %s", pageId)
	processedChildren, processedPage := pageMap[pageId]
	if !processedPage {

		addDomainArgs, removeDomainArgs, err := _propagateDomainsToSinglePage(db, pageId)
		if err != nil {
			return err
		}
	...
	if !processedChildren {
		...
		err := rows.Process(func(db *database.DB, rows *database.Rows) error {
			...
			err := propagateDomainsToPageAndDescendants(db, childId, pageMap)
			return err
		})
		if err != nil {
			return err
